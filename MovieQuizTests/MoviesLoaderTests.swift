...
//  MovieQuizTests
//
//  Created by Чингиз Джабаев on 08.08.2023.
//
...
struct StubNetworkClient: NetworkRouting {
    ...
    func fetch(url: URL, handler hendler: @escaping (Result<Data, Error>) -> Void) {
        if emulateError {
            hendler(.failure(TestError.test))
        } else {
            hendler(.success(expectedResponse))
        }
    }
